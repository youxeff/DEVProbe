...
from app.schemas.pull_request import ChangedFilesResponse, PullRequestResponse, PullRequestsResponse
...
@router.post("/{pr_number}", response_model=PullRequestResponse)
def get_pull_request(pr_number: Annotated[int, Path(gt=0)], payload: RepoUrlRequest):
    return github_service.fetch_pull_request(payload.repo_url, pr_number)
